...
export const processWebhookEvents = async (event) => {
  ...
  if (event?.action === JOB_IN_PROGRESS) {
    ...
    logger.info(
      { runnerName },
      "Workflow run in progress, picked up by runner"
    );
  ...
  if (event?.action === JOB_COMPLETED) {
    if (!event.workflow_job.runner_name) {
      logger.debug(
        "Not processing event for cancelled workflow run with no runner assigned"
      );
    ...
    logger.info(
      { runnerName: event.workflow_job.runner_name },
      "Enqueueing delete process for runner"
    );
