...
const createNetworkInterface = async (name) => {
  ...
  const [resourceGroupName, location, subnetId, runnerIdentifierLabel] =
    await Promise.all([
...
export const createVM = async (name) => {
  ...
  await client.virtualMachines.beginCreateOrUpdate(resourceGroupName, name, {
    identity: {
      type: "UserAssigned",
    ...
    storageProfile: {
      imageReference: (() => {
        switch (galleryImageType) {
          ...
          default:
            throw new Error(
              `Unsupported gallery image type: ${galleryImageType}\n` +
                `Supported types are: ${Object.values(GalleryType)
                  .map((value) => `'${value}'`)
                  .join(", ")}\n` +
...
export const deleteVM = async (name) => {
  ...
  await client.virtualMachines.beginDelete(resourceGroupName, name);
