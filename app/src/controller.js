...
export const reconcile = async (logger, event) => {
    ...
    if (event.action === WORKFLOW_QUEUED && runners.length === 0) {
        logger.debug("No runner available, about to execute createRunner");
        const name = await createRunner();
        logger.debug({ name }, "Created runner");
    }
    ...
    if (event.action === WORKFLOW_COMPLETED) {
        logger.debug("Workflow completed, about to execute deleteRunner");
        await deleteRunner(event.workflow_job.runner_name);
        logger.debug({ name: event.workflow_job.runner_name }, "Runner deleted");
    }
};
